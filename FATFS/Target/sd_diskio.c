...
#include "sd_diskio.h"
#include "sdmmc.h"      /* hsd1: SDMMC1 handle (polled reads below) */
...
{
  (void)lun;
  /* Manual fix: return the cached status instead of probing the card. */
  return Stat;
}
...
{
  (void)lun;
  /* Manual fix: polled (blocking) read. The CubeMX DMA path waits for a
   * "read complete" queue message that never arrives on this board, so every
   * read stalled for the full SD_TIMEOUT (30 s). A blocking HAL_SD_ReadBlocks
   * finishes in milliseconds and needs no DMA/ISR. */
  if (HAL_SD_ReadBlocks(&hsd1, buff, sector, count, SD_TIMEOUT) != HAL_OK)
  {
    return RES_ERROR;
  }
  return RES_OK;
