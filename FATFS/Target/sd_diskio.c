...
DRESULT SD_read(BYTE lun, BYTE *buff, DWORD sector, UINT count)
{
  ...
  /* Polling read: no DMA / interrupts required (CubeMX regenerated this file
   * with the DMA+RTOS template on 2026-08-30, but no SDMMC DMA is configured,
   * so reads failed with FR_DISK_ERR). Re-apply this if CubeMX regenerates. */
...
DRESULT SD_write(BYTE lun, const BYTE *buff, DWORD sector, UINT count)
{
  ...
  /* Polling write: no DMA / interrupts required. */
